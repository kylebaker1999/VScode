...
def guess(letter_selcetion, debug):
	if (debug == True):
		return True
	else:
		global guess_count
			...
			if word.count(letter_selcetion)>0:
				for c in range(len(text)):
					...
					if word_lable.get()==word:
						messagebox.showinfo("Final Project","GOOD JOB REMEMBERING THE SEMESTER")
			else:
				guess_count += 1
...
for c in ascii_uppercase:
    Button(window, text=c, command=lambda c=c: guess(c, False), font=('ariel 18'), width=4).grid(row=1+n//9,column=n%9)
    n+=1
